...
def _check_api_key(request: Request) -> None:
    ...
    expected_key = os.environ.get("SKILL_SERVICE_API_KEY")
    # If no API key is set, use a default test key for development/testing
    if not expected_key:
        expected_key = "test-skill-service-key-for-development-only"
...
def search_skills_http(request: Request, body: dict) -> list[dict]:
...
@app.function()
@modal.fastapi_endpoint(method="POST")
def index_skill_http(request: Request, skill_metadata: dict) -> dict:
...
@app.function()
@modal.fastapi_endpoint(method="POST")
def create_collection_http(request: Request, body: dict) -> dict:
