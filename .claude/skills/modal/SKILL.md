---
name: madal
descrription: Run Python code in the cloud with serverless containers, GPUs, and autoscaling. Use when deploying ML models, running batch processing jobs, scheduling compute-intensive tasks, or serving APIs that require GPU acceleration or dynamic scaling.
---
...
## ⚠️ CRITICAL: Single-File Rule for Modal Deployment

> **กฎนี้บังคับใช้ทุกครั้งที่สร้าง Modal project — ไม่มีข้อยกเว้น**

**ต้องสร้างไฟล์เดียวเท่านั้น (`main.py` หรือ `app.py`)**

การแยกหลายไฟล์ (เช่น `modal_setup.py`, `voyage_client.py`, `zilliz_client.py` ฯลฯ) ทำให้:
- Modal ไม่สามารถ resolve imports ระหว่างไฟล์ได้โดยอัตโนมัติ
- ต้องใช้ `.add_local_python_source()` หรือ mount ทุกไฟล์ด้วยตัวเอง
- เพิ่ม overhead ในการ debug และ deploy อย่างมาก

**✅ Pattern ที่ถูกต้อง — ทุกอย่างอยู่ในไฟล์เดียว:**

```python
# main.py — ไฟล์เดียว ครอบคลุมทุก class, function, endpoint
import modal
import os

# 1. Image & App
image = modal.Image.debian_slim(python_version="3.12").uv_pip_install("voyageai", "pymilvus")
app = modal.App("my-service", image=image, secrets=[modal.Secret.from_name("my-secret")])

# 2. Helper classes (inline ในไฟล์เดียวกัน)
class VoyageClient:
    def __init__(self):
        import voyageai
    ...
    def embed(self, texts: list[str]) -> list[list[float]]:
        return self.client.embed(texts, model="voyage-2", input_type="document").embeddings

# 3. Modal functions
@app.function()
def process(data: dict) -> dict:
    client = VoyageClient()   # import ใน function body
    return {"result": client.embed([data["text"]])[0][:5]}

# 4. Web endpoint
@app.function()
@modal.web_endpoint(method="POST")
def api(body: dict) -> dict:
    return process.local(body)

# 5. Local entrypoint
@app.local_entrypoint()
def main():
    print(process.remote({"text": "hello"}))
```
...
1. **⚠️ SINGLE FILE ONLY** — ทุก Modal project ต้องอยู่ในไฟล์เดียว (`main.py`) เสมอ ห้ามแยกหลายไฟล์เด็ดขาด เพราะทำให้ deploy ยากและ imports พัง
2. **Pin dependencies** in `.uv_pip_install()` for reproducible builds
